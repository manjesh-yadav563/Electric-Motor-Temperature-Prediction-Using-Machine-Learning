...
# Load your saved scaler and model (adjust filenames if different)
scaler = joblib.load('transformer.save')          # or 'transformer_sampled.save'
model = joblib.load('random_forest_model.save')       # or whatever name you used
...
def y_predict():
    try:
        # Get the 7 input values from the form (in the exact order your model was trained on)
...
if __name__ == '__main__':
    app.run(host='0.0.0.0', debug=True)  # debug=True for development only